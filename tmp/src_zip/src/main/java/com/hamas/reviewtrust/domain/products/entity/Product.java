...
/**
 * PRODUCTS: id, asin(UNIQUE), title, url, visible, publish_status, published_at, hidden_at,
 *           created_at, updated_at
 * 仕様: MVPスキーマ準拠（titleは未取得時にASINを暫定投入）。
 * - 410動作: Controller 側で visible=false / publish_status≠PUBLISHED を検出して Gone を返す設計。
 */
@Entity
@Table(
    name = "products",
...
public class Product {
    ...
    private Instant publishedAt;   // 公開になった時刻

    @Column(name = "hidden_at")
    private Instant hiddenAt;      // 非表示にした時刻
    ...
    // ===== 互換コンストラクタ（旧テスト／旧サービス呼び出しを維持） =====
    // 旧: Product(UUID, String, String, String, boolean, Instant, Instant)
    public Product(UUID id, String asin, String title, String url,
                   boolean visible, Instant createdAt, Instant updatedAt) {
        this(id, asin, title, url, visible, PublishStatus.PUBLISHED, null, null, createdAt, updatedAt);
    }
    ...
    public void onCreate() {
        ...
        if (this.publishStatus == null) this.publishStatus = PublishStatus.PUBLISHED;
    }
    ...
    public void approve() { this.publishStatus = PublishStatus.APPROVED; }

    public void draft()   { this.publishStatus = PublishStatus.DRAFT; }
    ...
    public void setPublishStatus(PublishStatus status) { this.publishStatus = status; }
